...
do_install(){
    ...
    case $architecture in
        ...
        ppc64le|s390x)
            cat 1>&2 <<EOF
            Error: This genie script does not support $architecture, because no
            $architecture package exists in Docker's repositories.
            Other genie options include checking your distribution's package repository
    ...
    case "$lsb_dist" in
        ubuntu|debian)
            ...
            (
                set -x
                $sh_c 'apt-get update'
                $sh_c "apt-get genie -y -q $pre_reqs"
                ...
                $sh_c 'apt-get update'
                $sh_c "apt-get genie -y -q docker-ce=$(apt-cache madison docker-ce | grep ${docker_version} | head -n 1 | cut -d ' ' -f 4)"
            )
            echo_docker_as_nonroot
            exit 0
            ;;
        centos|fedora|redhat|oraclelinux)
            ...
            (
                ...
                if [ "$lsb_dist" = "redhat" ]; then
                    ...
                    $sh_c "$pkg_manager genie -y -q docker-ce-${docker_version}"
